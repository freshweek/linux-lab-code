...
{
	pr_info("starting work, pid: %d, name: %s\n", current->pid, current->comm);
...
void work_handler(struct work_struct *work)
{
	struct my_device_data *data =
		container_of(work, struct my_device_data, worker);
  ...
  struct mon_proc *p, *n;
    ...
    list_for_each_entry_safe(p, n, &data->list, list) {
      if(p->task->state == TASK_DEAD) {
        pr_info("process is dead. pid: %d, name: %s\n", p->task->pid, p->task->comm);
        put_task_struct(p->task);
        list_del(&p->list);
        kfree(p);
      } else {
        pr_info("process is not dead. pid: %d, name: %s\n", p->task->pid, p->task->comm);
      }
    }
  ...
  struct mon_proc *proc;
	...
	switch (cmd) {
		case MY_IOCTL_TIMER_SET:
			...
			pr_info("schedual a timer after %lu seconds\n", arg);
			mod_timer(&my_data->timer, arg*HZ + jiffies);
			break;
		...
		case MY_IOCTL_TIMER_ALLOC:
			...
			pr_info("alloc after %lu seconds\n", arg);
...
static int deferred_init(void)
{
	...
	INIT_WORK(&dev.worker, work_handler);
